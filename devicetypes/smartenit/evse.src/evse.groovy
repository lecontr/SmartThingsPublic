...
metadata {
	definition (name: "EVSE", namespace: "Smartenit", author: "Luis Contreras", mnmn: "SmartThingsCommunity", vid: "5202137a-81dc-3b9e-89fb-b17215a6ed2b", ocfDeviceType: "oic.d.smartplug") {
		...
		fingerprint model: "IOTEVSE-Z", manufacturer: "Smartenit, Inc", deviceJoinName: "SMRT-EVSE"
	}
}
...
def parse(String description) {
	...
	if (event) {
    	...
    	if (event.name == "power") {
        	log.debug "event name is power"
            sendEvent(name: "power", value: (event.value/EnergyDivisor))
        }
        else {
        	log.debug "event name is not power"
            sendEvent(event)
        }
    }
    else {
        ...
        if(mapDescription) {
            if(mapDescription.cluster == "0702") {
                ...
                } else if(mapDescription.attrId == "0016") {
...
def setAutoStart(val) {
    ...
    def newVal = zigbee.convertHexToInt(val)
    log.debug "this is the new val: ${newVal}"
...
def refresh() {
    ...
    zigbee.readAttribute(EVSECluster, ChargerDeliveredSummation, [mfgCode: SmartenitMfrCode])
}
